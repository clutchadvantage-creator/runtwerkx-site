...
const DEFAULT_PAGE_URL = 'https://runtwerkx.com/knowledge-library/construction/worksheets'
const DEFAULT_PAGE_BACKGROUND_IMAGE_SRC = '/images/construction.png'
const DEFAULT_DRAFT_STORAGE_KEY = 'runtwerkx-construction-worksheet-drafts'
const DEFAULT_OFFLINE_ACTION_QUEUE_KEY = 'runtwerkx-construction-offline-action-queue'
const DEFAULT_OFFLINE_ACTION_HISTORY_KEY = 'runtwerkx-construction-offline-action-history'
...
function createPdfBlob(worksheet, draft, qualityMode, pdfHeaderTitle) {
  const doc = exportWorksheetPdf(worksheet, draft, { shouldSave: false, qualityMode, pdfHeaderTitle })
  return doc.output('blob')
}

function createJsonPayloadForWorksheet(worksheet, draft, sourceLabel) {
  return {
    version: 1,
...
export default function IndustryWorksheetsPage({ config = {} }) {
  const importPackInputRef = useRef(null)
  const importWorksheetInputRef = useRef(null)
  const worksheetDefinitions = config.worksheets?.length ? config.worksheets : WORKSHEETS
  const pageUrl = config.pageUrl || DEFAULT_PAGE_URL
  const pageBackgroundImageSrc = config.backgroundImageSrc || DEFAULT_PAGE_BACKGROUND_IMAGE_SRC
  const draftStorageKey = config.draftStorageKey || DEFAULT_DRAFT_STORAGE_KEY
  const offlineActionQueueKey = config.offlineActionQueueKey || DEFAULT_OFFLINE_ACTION_QUEUE_KEY
  const offlineActionHistoryKey = config.offlineActionHistoryKey || DEFAULT_OFFLINE_ACTION_HISTORY_KEY
  const sourceLabel = config.sourceLabel || 'RuntWerkx Construction Worksheets'
  const pdfHeaderTitle = config.pdfHeaderTitle || 'RuntWerkx Construction Worksheet'
  const metaDescription =
    config.metaDescription ||
    'Fill, sign, validate, print, export branded PDFs, and move worksheet drafts for daily construction forms including site checklists, pre-task plans, pour logs, delivery inspections, and equipment checks.'
  const parentPath = config.parentPath || '/knowledge-library/construction'
  const parentLabel = config.parentLabel || 'Back to Construction'
  const heroEyebrow = config.heroEyebrow || '― Construction Tools ―'
  const heroTitle = config.pageTitle || 'Construction Worksheets'
  ...
  const libraryEyebrow = config.libraryEyebrow || '― Worksheet Library ―'
  const libraryTitle = config.libraryTitle || 'Daily construction forms crews can fill, sign, and move'
  const libraryDescription =
    config.libraryDescription ||
    'Pick a worksheet, fill it directly in the browser, capture real handwritten signatures, validate critical fields, export a branded PDF, move either one worksheet or the full draft pack between devices, print it, or download a filled HTML copy.'
  const draftPackFilenamePrefix = config.draftPackFilenamePrefix || 'runtwerkx-construction-drafts'
  const [activeKey, setActiveKey] = useState(worksheetDefinitions[0].key)
  const [isOnline, setIsOnline] = useState(() => (typeof navigator === 'undefined' ? true : navigator.onLine))
  const [exportQuality, setExportQuality] = useState('high')
  const [pendingErrorTarget, setPendingErrorTarget] = useState(null)
  const [drafts, setDrafts] = useState(() => {
    if (typeof window === 'undefined') {
      return worksheetDefinitions.reduce((accumulator, worksheet) => {
    ...
    try {
      const savedDrafts = JSON.parse(window.localStorage.getItem(draftStorageKey) || '{}')
  ...
  const [queuedActions, setQueuedActions] = useState(() => {
    ...
    try {
      return JSON.parse(window.localStorage.getItem(offlineActionQueueKey) || '[]')
    } catch {
      return []
    }
  })
  const [queueHistory, setQueueHistory] = useState(() => {
    ...
    try {
      return JSON.parse(window.localStorage.getItem(offlineActionHistoryKey) || '[]')
    } catch {
      return []
    }
  })
  ...
  const activeWorksheet = useMemo(
    () => worksheetDefinitions.find((worksheet) => worksheet.key === activeKey) || worksheetDefinitions[0],
    [activeKey, worksheetDefinitions]
  )
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(offlineActionQueueKey, JSON.stringify(queuedActions))
    return undefined
  }, [queuedActions])
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(offlineActionHistoryKey, JSON.stringify(queueHistory))
    return undefined
  }, [queueHistory])
  ...
  function updateWorksheetDraft(worksheetKey, fieldId, value) {
    setDrafts((currentDrafts) => {
      const worksheet = worksheetDefinitions.find((item) => item.key === worksheetKey) || activeWorksheet
      return {
        ...currentDrafts,
        [worksheetKey]: {
  ...
  async function processQueuedAction(queueItem) {
    const worksheet = worksheetDefinitions.find((item) => item.key === queueItem.worksheetKey)
    if (!worksheet) {
    ...
    if (queueItem.type === 'share-json') {
      const payload = createJsonPayloadForWorksheet(worksheet, draftSnapshot, sourceLabel)
    ...
    if (queueItem.type === 'share-pdf') {
      const blob = createPdfBlob(worksheet, draftSnapshot, exportQuality, pdfHeaderTitle)
      ...
      exportWorksheetPdf(worksheet, draftSnapshot, { qualityMode: exportQuality, pdfHeaderTitle })
      setQueuedActions((currentQueue) => currentQueue.filter((item) => item.id !== queueItem.id))
  ...
  async function handleShareWorksheetJson(worksheet) {
    const payload = createJsonPayloadForWorksheet(worksheet, drafts[worksheet.key] || createEmptyWorksheetDraft(worksheet), sourceLabel)
    if (!isOnline) {
  ...
  async function handleShareWorksheetPdf(worksheet) {
    ...
    const blob = createPdfBlob(worksheet, drafts[worksheet.key] || createEmptyWorksheetDraft(worksheet), exportQuality, pdfHeaderTitle)
    const file = new File([blob], `${worksheet.key}.pdf`, { type: 'application/pdf' })
    const wasShared = await shareWorksheetFile({
      file,
      title: `${worksheet.title} PDF`,
  ...
  function handleExportDrafts() {
    downloadJson(
      ...
      `${draftPackFilenamePrefix}-${new Date().toISOString().slice(0, 10)}.json`
    )
  ...
  function handleExportSingleWorksheet(worksheet) {
    downloadJson(createJsonPayloadForWorksheet(worksheet, drafts[worksheet.key] || createEmptyWorksheetDraft(worksheet), sourceLabel), `${worksheet.key}-draft.json`)
  ...
  function handleImportSingleWorksheet(event) {
    ...
    reader.onload = () => {
      try {
        ...
        const targetWorksheet = worksheetDefinitions.find((worksheet) => worksheet.key === worksheetKey)
  ...
  function handleEmail(worksheet) {
    ...
    const subject = encodeURIComponent(`${worksheet.title} | ${sourceLabel}`)
    const body = encodeURIComponent(
      `Open this ${(config.industryName || 'construction').toLowerCase()} worksheet page for ${worksheet.title}: ${window.location.href}\n\nYou can also export a branded PDF directly from the worksheet panel before sending.`
    )
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black text-white">
      <Helmet>
        <title>{heroTitle} | RuntWerkx</title>
        <meta
          name="description"
      ...
      <div
        ...
        style={{ backgroundImage: pageBackgroundImageSrc ? `url(${pageBackgroundImageSrc})` : 'none' }}
      />
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-7xl px-6 pb-16 pt-20">
            ...
            <div className="mx-auto max-w-5xl rounded-[2rem] bg-black/55 px-6 py-12 text-center backdrop-blur-[2px] md:px-10 md:py-12">
              <p className="text-sm font-semibold uppercase tracking-[0.30em] text-green-400">{heroEyebrow}</p>
              <h1 className="mt-5 text-4xl font-black leading-tight text-white md:text-6xl">{heroTitle}</h1>
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-6 py-14">
          ...
          <div className="mt-12 grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
            <div className="space-y-5">
              {worksheetDefinitions.map((worksheet) => {
